...
def _create_financing(
    ...
    response = client.post(
        f"/budget/profiles/{profile_id}/property-financings",
        json={
            "name": name,
...
def _create_entry(
    ...
    *,
    ...
    response = client.post(
        f"/budget/profiles/{profile_id}/property-financings/{financing_id}/entries",
        json={
...
def test_get_snapshot_empty(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN Empty"}).json()["id"]
    response = client.get(f"/budget/profiles/{profile_id}/property-financings")
...
def test_get_snapshot_404_unknown_portfolio(client: TestClient) -> None:
    response = client.get("/budget/profiles/99999/property-financings")
    assert response.status_code == 404


def test_create_financing_and_entry_metrics(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN HTTP"}).json()["id"]
    financing = _create_financing(client, profile_id)
    ...
    snapshot = client.get(f"/budget/profiles/{profile_id}/property-financings").json()
...
def test_entry_category_must_match_type(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN Val"}).json()["id"]
    financing = _create_financing(client, profile_id)
    response = client.post(
        f"/budget/profiles/{profile_id}/property-financings/{financing['id']}/entries",
        json={
...
def test_financings_isolated_by_portfolio(client: TestClient) -> None:
    p1 = client.post("/budget/profiles", json={"name": "FIN P1"}).json()["id"]
    p2 = client.post("/budget/profiles", json={"name": "FIN P2"}).json()["id"]
    _create_financing(client, p1, name="Imóvel P1")

    snap1 = client.get(f"/budget/profiles/{p1}/property-financings").json()
    snap2 = client.get(f"/budget/profiles/{p2}/property-financings").json()
    assert len(snap1["financings"]) == 1
    assert len(snap2["financings"]) == 0


def test_duplicate_name_rejected(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN Dup"}).json()["id"]
    _create_financing(client, profile_id)
    response = client.post(
        f"/budget/profiles/{profile_id}/property-financings",
...
def test_delete_financing(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN Del"}).json()["id"]
    financing = _create_financing(client, profile_id)
    response = client.delete(
        f"/budget/profiles/{profile_id}/property-financings/{financing['id']}"
    )
...
def _create_entry_template(
    ...
    *,
    ...
    response = client.post(
        f"/budget/profiles/{profile_id}/property-financings/{financing_id}/entry-templates",
        json={
            "name": name,
            "entry_type": entry_type,
            "event_category": event_category,
...
def test_create_entry_template_and_snapshot(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN Tpl"}).json()["id"]
    financing = _create_financing(client, profile_id)
    template = _create_entry_template(client, profile_id, financing["id"])
...
def test_entry_template_category_must_match_type(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN Tpl Val"}).json()["id"]
    financing = _create_financing(client, profile_id)
    response = client.post(
        f"/budget/profiles/{profile_id}/property-financings/{financing['id']}/entry-templates",
        json={
            "name": "Inválido",
            "entry_type": "income",
            "event_category": "financiamento",
...
def test_duplicate_template_name_rejected(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN Tpl Dup"}).json()["id"]
    financing = _create_financing(client, profile_id)
    _create_entry_template(client, profile_id, financing["id"])
    response = client.post(
        f"/budget/profiles/{profile_id}/property-financings/{financing['id']}/entry-templates",
        json={
...
def test_update_entry_template(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN Tpl Upd"}).json()["id"]
    financing = _create_financing(client, profile_id)
    template = _create_entry_template(client, profile_id, financing["id"])
    response = client.patch(
        f"/budget/profiles/{profile_id}/property-financings/entry-templates/{template['id']}",
...
def test_delete_entry_template(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN Tpl Del"}).json()["id"]
    financing = _create_financing(client, profile_id)
    template = _create_entry_template(client, profile_id, financing["id"])
    response = client.delete(
        f"/budget/profiles/{profile_id}/property-financings/entry-templates/{template['id']}"
...
def test_entry_template_isolated_by_portfolio(client: TestClient) -> None:
    p1 = client.post("/budget/profiles", json={"name": "FIN Tpl P1"}).json()["id"]
    p2 = client.post("/budget/profiles", json={"name": "FIN Tpl P2"}).json()["id"]
    financing = _create_financing(client, p1)
    template = _create_entry_template(client, p1, financing["id"])
    response = client.patch(
        f"/budget/profiles/{p2}/property-financings/entry-templates/{template['id']}",
        json={"amount_brl": 1.0},
    )
    assert response.status_code == 404
...
def test_delete_financing_cascades_templates(client: TestClient) -> None:
    profile_id = client.post("/budget/profiles", json={"name": "FIN Tpl Cas"}).json()["id"]
    financing = _create_financing(client, profile_id)
    _create_entry_template(client, profile_id, financing["id"])
    response = client.delete(
        f"/budget/profiles/{profile_id}/property-financings/{financing['id']}"
