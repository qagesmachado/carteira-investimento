# Financiamento imóvel — casos E2E

Prefixo: `UI-FERR-`

| ID | Arquivo spec | Status |
| --- | --- | --- |
| UI-FERR-001 | `01-resumo-vazio.spec.ts` | aprovado |
| UI-FERR-002 | `02-criar-financiamento.spec.ts` | aprovado |
| UI-FERR-003 | `03-lancar-mes.spec.ts` | aprovado |
| UI-FERR-004 | `04-chamada-capital.spec.ts` | aprovado |
| UI-FERR-005 | `05-kpis-grafico.spec.ts` | aprovado |
| UI-FERR-006 | `06-multiplos-imoveis.spec.ts` | aprovado |
| UI-FERR-007 | `07-trocar-carteira.spec.ts` | aprovado |

Referência: [controle-financiamento-imovel.md](../../../../docs/produto/desenvolvido/controle-financiamento-imovel.md)
