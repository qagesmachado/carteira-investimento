...
- **ID:** `UI-DAD-008`
- **Status:** aprovado
- **Página:** `/portfolios` (ação) · `/dashboard` e `/consolidada` (verificação)
...
2. Em `/consolidada`, com a carteira ativa, expando detalhes de `KLBN` (se ainda listado) ou verifico totais de proventos no painel/resumo global.
...
7. Em `/consolidada`, `KLBN` não aparece mais como posição aberta; o resumo de proventos da carteira (ou do ativo no histórico, conforme UI) **ainda reflete** R$ 25,00 ou equivalente documentado.
