<script lang="ts">
  ...
  async function loadMethodology() {
    ...
    const result = await getAnalysisMethodology('etf-intl', activeId);
  ...
  async function handleMethodologyChange(next: AnalysisMethodology) {
    ...
    error = '';
    try {
      const result = await saveAnalysisMethodology('etf-intl', activeId, next);
      methodology = result.methodology;
      await reloadAllocation();
    } catch (err) {
...
<svelte:head>
  <title>Análise — Internacional</title>
</svelte:head>
...
<div class="flex flex-col gap-3">
  ...
  <AnalysisSimpleAllocationPanel
    ...
    sectionTitle="ETFs internacionais"
    sectionTestId="analysis-internacional-section"
    filterTestId="analysis-internacional-table-filter"
    description="Defina o percentual desejado de cada ETF (soma 100%) e opcionalmente um link externo de análise."
    emptyMessage="Nenhum ETF internacional na carteira ativa. Adicione posições em USD (ex.: VOO) em Carteiras."
    classDisplayClass="international"
    rebalanceAssetsKey="international_assets"
    showUsdColumn={true}
    {activeId}
    {positions}
    {loading}
    listAnalysis={listEtfIntlAnalysis}
    saveAllocations={saveEtfIntlAllocations}
  />
</div>
