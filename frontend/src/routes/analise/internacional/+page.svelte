<script lang="ts">
  ...
  $: internationalClassTarget =
    snapshot?.classes.find((c) => c.display_class === 'international')?.target_percent ?? 20;
  $: patrimonyBrl = snapshot?.patrimony_brl ?? 0;
  $: usdRate = snapshot?.usd_brl_rate ?? null;
  $: groupCurrentTotal = (snapshot?.international_assets ?? [])
    .filter((a) => assetIdsInPortfolio.has(a.asset_id))
    .reduce((sum, a) => sum + a.current_value_brl, 0);

  $: rebalanceByAssetId = new Map(
    (snapshot?.international_assets ?? []).map((a) => [a.asset_id, a])
  );
  ...
  function buildDisplayRows(
    ...
    return analysisRows.map((row) => {
      ...
      const desiredBrl = computeDesiredValueBrl(
        patrimonyBrl,
        internationalClassTarget,
        targetPercent
      );
  ...
  async function handleSave() {
    ...
    message = '';
    try {
      const updated = await saveEtfIntlAllocations(
        drafts.map((d) => ({
          asset_id: d.asset_id,
  ...
  <section class="card bg-base-100 shadow">
    <div class="card-body gap-4">
      <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 class="card-title text-lg">ETFs internacionais</h2>
          <p class="text-sm text-base-content/70">
            Defina o percentual desejado de cada ETF (soma 100%) e opcionalmente um link externo de
            análise.
          </p>
        </div>
      ...
      {:else if portfolioRows.length === 0}
        <p class="text-sm text-base-content/70">
          Nenhum ETF internacional na carteira ativa. Adicione posições em USD (ex.: VOO) em
          Carteiras.
        </p>
      {:else}
