<script lang="ts">
  ...
  async function handleSave() {
    ...
    message = '';
    try {
      config = await saveStockBrConfig({
  ...
  async function handleReset() {
    if (!confirm('Restaurar configuração padrão da coluna Soma?')) return;
    saving = true;
    error = '';
    message = '';
    try {
      config = await resetStockBrConfig();
      tableDisplay = structuredClone(config.table_display);
...
<section class="card bg-base-100 shadow">
  <div class="card-body gap-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h2 class="card-title">Coluna Soma</h2>
    ...
    {:else}
      <p class="text-sm text-base-content/70">
        Lucros + Dívida + Tag along + Segmento + peso de viabilidade + (multiplicador × diagrama).
      </p>
