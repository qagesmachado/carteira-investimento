<script lang="ts">
  ...
  let lastLoadedProfileId: number | null = null;
  ...
  async function loadSnapshot(id: number) {
    ...
    error = '';
    try {
      snapshot = await getPropertyFinancingSnapshot(id);
      if (
  ...
  async function handleFinancingSave(
    ...
    try {
      if (editMode === 'create') {
        const created = await createPropertyFinancing(profileId, {
          name: event.detail.name,
          property_type: event.detail.property_type,
          description: event.detail.description || null
        });
        editModalOpen = false;
        await loadSnapshot(profileId);
        selectedFinancingId = created.id;
      } else if (selectedFinancing) {
        await updatePropertyFinancing(profileId, selectedFinancing.id, {
          name: event.detail.name,
          property_type: event.detail.property_type,
          description: event.detail.description || null
        });
  ...
  async function handleDeleteFinancing() {
    if (profileId == null || !selectedFinancing) return;
    if (!confirm(`Excluir o financiamento «${selectedFinancing.name}»?`)) return;
    saving = true;
    error = '';
    try {
      await deletePropertyFinancing(profileId, selectedFinancing.id);
      selectedFinancingId = FINANCING_SUMMARY_TAB_ID;
      await loadSnapshot(profileId);
    } catch (err) {
  ...
  async function handleAddEntry(event: CustomEvent<FinancingEntryCreate>) {
    ...
    error = '';
    try {
      await createFinancingEntry(profileId, selectedFinancing.id, event.detail);
      await loadSnapshot(profileId);
    } catch (err) {
  ...
  async function handleUpdateEntry(
    ...
    error = '';
    try {
      await updateFinancingEntry(profileId, event.detail.entryId, {
  ...
  async function handleDeleteEntry(event: CustomEvent<number>) {
    ...
    error = '';
    try {
      await deleteFinancingEntry(profileId, event.detail);
      await loadSnapshot(profileId);
    } catch (err) {
  ...
  async function handleCreateTemplate(event: CustomEvent<FinancingEntryTemplateCreate>) {
    ...
    try {
      await createFinancingEntryTemplate(profileId, selectedFinancing.id, event.detail);
  ...
  async function handleDeleteTemplate(event: CustomEvent<number>) {
    ...
    error = '';
    try {
      await deleteFinancingEntryTemplate(profileId, event.detail);
...
{#if profileId == null}
  <EmptyStateCallout {...NO_BUDGET_PROFILE_EMPTY_STATE} />
