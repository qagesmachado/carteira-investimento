...
Permitir alternar entre tema **claro** (`light`) e **escuro** (`dim`) em **todas as telas** da aplicação, com preferência persistida localmente.
...
- Temas DaisyUI: `light` (padrão) e `dim` (escuro suave).
...
| Clique no ícone lua | Ativa tema `dim` |
...
4. Validar visualmente em `light` e `dim` antes do merge.
...
- Config DaisyUI: `frontend/tailwind.config.js` (`themes: ['light', 'dim']`)
